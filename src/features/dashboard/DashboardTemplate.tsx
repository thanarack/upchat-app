...
const DashboardTemplate = (props: any) => {
  ...
  const { userSetLogout, user } = useAuth();
  const { myRooms, roomsAddNewRoom } = useRooms();
  ...
  const getProfileUrl = (userId: any) => {
    let result = '';
    const getUserFromContact = user.contacts.find(
      (data: any) => data.id === userId
    );
    if (getUserFromContact) {
      result = getUserFromContact.profileUrl;
    }
    return result;
  };
